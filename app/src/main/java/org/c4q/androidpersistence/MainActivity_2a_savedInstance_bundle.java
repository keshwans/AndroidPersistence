...
public class MainActivity_2a_savedInstance_bundle extends ActionBarActivity {
    ...
    private void saveDataToBundle(Bundle outState) {
        outState.putInt("user_count", mUserCount);
    }

    private void restoreDataFromBundle(Bundle outState) {
        if (outState != null) {
            mUserCount = outState.getInt("user_count", 0);  // TODO: refactor this string to a constant, so there is no mismatch between key used for put and get methods
        } else {
            mUserCount = 0;
        }
    }
